...
#include <inttypes.h>
#include <RTC_ALIAS.h>
#define ADI_RTC                                 pADI_rtc1
#define ADI_RTC_BASE                            ADI_rtc1_BASE
...
//========================================
//
#define WERRCODE_RTCCR   ((&pADI_rtc1->CR0 >> 2) & 0x0000000F)
#define WERRCODE_RTCSR   ((&pADI_rtc1->SR0 >> 2) & 0x0000000F)
#define WERRCODE_RTCSR0  ((&pADI_rtc1->SR0 >> 2) & 0x0000000F)
#define WERRCODE_RTCSR1  ((&pADI_rtc1->SR1 >> 2) & 0x0000000F)
#define WERRCODE_RTCCNT0 ((&pADI_rtc1->CNT0 >> 2) & 0x0000000F)
#define WERRCODE_RTCCNT1 ((&pADI_rtc1->CNT1 >> 2) & 0x0000000F)
#define WERRCODE_RTCALM0 ((&pADI_rtc1->ALM0 >> 2) & 0x0000000F)
#define WERRCODE_RTCALM1 ((&pADI_rtc1->ALM1 >> 2) & 0x0000000F)
#define WERRCODE_RTCTRM  ((&pADI_rtc1->TRM >> 2) & 0x0000000F)

#define WERRCODE_NOERR    0xF
#define WERRCODE_FIFOFULL ((1 << 3) | (((int)(&pADI_rtc1->CR0) >> 2) & 0x00000007))
#define WERRCODE_WDOG     ((1 << 3) | (((int)(&pADI_rtc1->SR0) >> 2) & 0x00000007)) 
...
{
    Nop(50);
    //while((pADI_rtc1->SR0 & 0x3F80) != 0x3F80); // we are only interested in sync bits
    //while((pADI_rtc1->SR2) & 0xC000 != 0xC000);
    while(~pADI_rtc1->SR0 & 0x3F80); // we are only interested in sync bits
    while(~pADI_rtc1->SR2 & 0xC000);
}
void wait_pend(void)
{
    //while((pADI_rtc1->SR1) & 0x3F80 != 0x3F80); // wait until all writes are no longer pending
    //while((pADI_rtc1->SR2) & 0x3000 != 0x3000);
    while(pADI_rtc1->SR1 & 0x3F80); // wait until all writes are no longer pending
    while(pADI_rtc1->SR2 & 0x3000);
}
...
#define LOCK_ISO (RTC_SR0_LOCK32KHZ_Msk | RTC_SR0_ISOENB_Msk)
    ...
    do {
        ...
        // 
        pADI_rtc1->DBG = RTC_DEBUG_UNLOCK_KEY1;
        pADI_rtc1->DBG = RTC_DEBUG_UNLOCK_KEY2;
        SR0_MASKED = pADI_rtc1->SR0 & LOCK_ISO;
...
{
    unlock_rtc_debug();
    pADI_rtc1->DBG = RTC_DBG_DISPRESCALE;

}
void no_debug_8khz_mode(void)
{
    unlock_rtc_debug();
    pADI_rtc1->DBG = RTC_DBG_NODISPRESCALE;
...
void enable_32k_clock()
{
    ...
    pADI_CLK->OSCKEY = 0xCB14;
    pADI_CLK->OSCCTRL |= BITM_CLK_OSCCTRL_LFCLKMUX;
}
#endif
